...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    const { data, updateReloadeState } = props;
        ...
        return data.map((item) => {
            return (
                <tr key={item._id} className="border-b text-white bg-gray-600 hover:bg-white hover:text-gray-800">
                    <td className="px-6 py-4 break-words"><Link to={item.fullUrl} target="_blank" rel="noreferrer noopener">{item.fullUrl}</Link></td>
                    <td className="px-6 py-4 "><Link to={`${serverUrl}/shorturl/${item.shortUrl}`} target="_blank" rel="noreferrer noopener">{item.shortUrl}</Link></td>
                    <td className="px-6 py-4 ">{item.clicks}</td>
                    <td className="px-6 py-4 ">
                        <div className="flex content-center">
                            <div className="cursor-pointer px-s" onClick={() => copyToClipBoard(item.shortUrl)}>
                            ...
                            <div className="cursor-pointer px-s" onClick={() => deleteUrl(item._id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
